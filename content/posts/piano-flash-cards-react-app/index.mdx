...
My wife and I recently got a piano and are learning how to play. I used this as an excuse to build another app with [Vexflow](https://vexflow.com/).
I developed a flash card app to practice reading notes on the treble clef and bass clef in different key signatures.
...
In this post I'll share how I built the app and some things I learned along the way.
...
make it more fun by adding a score concept and 3 different difficultly levels. Here are the concepts of the piano flash card game:

- Every time the user chooses the right note their score increases by one.
...
When building this flash card app, I used Vexflow for two things:
...
This app manages the following data which is stored in memory with `useState()`:
...
I recently watched this ["Goodbye, useEffect" Video](https://www.youtube.com/watch?v=bGzanfKVFeU) and decided to apply it to my app. Initially I focused on getting this flash card app working and I reached for useEffect to synchronize my game state.

At first, I had the Next Note button event handler update the GameState only and then had the useEffect hook react to it and finish up the rest of the logic. Here's a simplified example,
...
I learned from this talk that event handlers are a better place to manage side effects and make state updates. By doing so I was able to get rid of useEffect alltogether.
...
- It makes it easy to build responsive apps. Design your mobile layout first and then add specific classes for larger widths (for example, increasing vertical padding on larger devices with: `sm:py-8 lg:py-10`)
